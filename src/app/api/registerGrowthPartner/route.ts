...
import { registerPublicGrowthPartner } from '@/features/growth-partners/actions/register-public-growth-partner';
import { PUBLIC_INTAKE } from '@/config/public-intake';

/**
 * Website → CRM Growth Partner Registration intake (Doc 25 §4 step 1),
 * mirrors `/api/createLead`.
 *
 ...
 * `createLead` — origin allow-list, honeypot, rate limits.
 */
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const result = await registerPublicGrowthPartner(body, {
    ip: clientIp(request),
