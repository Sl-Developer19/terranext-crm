...
/**
 * Table primitives (Doc 06 §3). The full server-paginated DataTable wrapper
 * arrives with the first large list module (leads); admin-scale tables use
...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn('border-b transition-colors hover:bg-gold/5', className)}
      {...props}
    />
  ),
);
